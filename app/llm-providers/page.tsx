...
export default function LLMProviders() {
  ...
  async function handleSubmit() {
    if (editing) {
      await fetch("/api/llm-providers", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, id: editing.id }) })
    } else {
      await fetch("/api/llm-providers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
    }
    setShowForm(false)
    setEditing(null)
    setForm({ name: "" })
  ...
  async function handleDelete(id: string) {
    ...
    await fetch("/api/llm-providers", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) })
    fetchData()
  }
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-8">
      ...
      <h1 className="text-3xl font-black text-white mb-8">🏢 LLM Providers</h1>

      {showForm && (
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 mb-8">
          <h2 className="text-white font-bold text-lg mb-4">{editing ? "Edit Provider" : "New Provider"}</h2>
          <div>
      ...
      <AdminTable title="LLM Providers" emoji="🏢" data={data} columns={["id", "name", "created_datetime_utc"]}
        onDelete={handleDelete} onEdit={handleEdit} onCreate={() => setShowForm(true)} createLabel="Add Provider" />
    </main>
  )
}