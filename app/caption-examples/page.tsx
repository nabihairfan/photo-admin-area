...
export default function CaptionExamples() {
  const router = useRouter()
  const [data, setData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<any>(null)
  const [form, setForm] = useState({ caption: "", image_description: "", explanation: "", priority: "" })
  ...
  async function fetchData() {
    const res = await fetch("/api/caption-examples")
  ...
  async function handleSubmit() {
    if (editing) {
      await fetch("/api/caption-examples", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, id: editing.id }) })
    } else {
      await fetch("/api/caption-examples", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
    }
    setShowForm(false)
    setEditing(null)
    setForm({ caption: "", image_description: "", explanation: "", priority: "" })
    fetchData()
  }

  async function handleDelete(id: string) {
    if (!confirm("Delete this caption example?")) return
    await fetch("/api/caption-examples", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) })
    fetchData()
  }

  function handleEdit(row: any) {
    setEditing(row)
    setForm({ caption: row.caption, image_description: row.image_description, explanation: row.explanation, priority: row.priority })
    setShowForm(true)
  }
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-8">
      ...
      <h1 className="text-3xl font-black text-white mb-8">✍️ Caption Examples</h1>

      {showForm && (
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 mb-8">
          <h2 className="text-white font-bold text-lg mb-4">{editing ? "Edit Caption Example" : "New Caption Example"}</h2>
          <div className="grid grid-cols-2 gap-4">
            {["caption", "image_description", "explanation", "priority"].map((field) => (
              <div key={field}>
      ...
      <AdminTable title="Caption Examples" emoji="✍️" data={data} columns={["id", "caption", "image_description", "explanation", "priority"]}
        onDelete={handleDelete} onEdit={handleEdit} onCreate={() => setShowForm(true)} createLabel="Add Example" />
    </main>
  )
}