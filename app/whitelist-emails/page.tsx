...
export default function WhitelistEmails() {
  const router = useRouter()
  const [data, setData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<any>(null)
  const [form, setForm] = useState({ email_address: "" })
  ...
  async function handleSubmit() {
    if (editing) {
      await fetch("/api/whitelist-emails", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, id: editing.id }) })
    } else {
      await fetch("/api/whitelist-emails", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
    }
    setShowForm(false)
    setEditing(null)
    setForm({ email_address: "" })
    fetchData()
  }

  async function handleDelete(id: string) {
    if (!confirm("Delete this email?")) return
    await fetch("/api/whitelist-emails", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) })
    fetchData()
  }

  function handleEdit(row: any) {
    setEditing(row)
    setForm({ email_address: row.email_address })
    setShowForm(true)
  }
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-8">
      ...
      <h1 className="text-3xl font-black text-white mb-8">📧 Whitelisted Emails</h1>

      {showForm && (
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 mb-8">
          <h2 className="text-white font-bold text-lg mb-4">{editing ? "Edit Email" : "New Email"}</h2>
          <div>
            <label className="text-gray-400 text-xs uppercase mb-1 block">Email Address</label>
            <input className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500"
              placeholder="e.g. user@columbia.edu" value={form.email_address} onChange={(e) => setForm({ email_address: e.target.value })} />
          </div>
      ...
      <AdminTable title="Whitelisted Emails" emoji="📧" data={data} columns={["id", "email_address", "created_datetime_utc"]}
        onDelete={handleDelete} onEdit={handleEdit} onCreate={() => setShowForm(true)} createLabel="Add Email" />
    </main>
  )
}